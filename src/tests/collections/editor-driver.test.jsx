 ...
 * holding (see the `active` / `mirror` contract in
 * collections/hooks/use-collection-editor.js).
 *
 * The election works where it is written down: CollectionItem stands the page
 * scope down unless it is the elected one, admin/BlockCard stands the drawer
 * card down while page fields exist. The rail's detail pane calls
 * `useCollectionEditor` with bare defaults (admin/CollectionRegionPanel), so it
 * always claims `active: true` — case A is that hole.
 *
 ...
 * (CollectionComposer) and the rail's create pane (admin/CollectionRegionPanel)
 * pass the default `true` while POSTing to the same
 * `/cms/collections/{key}/drafts` slot.
 *
 * Two surfaces is the ceiling, not three: the drawer's `mode === "collections"`
 * branch is exclusive (admin/Drawer), so its Page-tab Collection card and the
 * rail's detail pane can never be mounted at the same time.
 */

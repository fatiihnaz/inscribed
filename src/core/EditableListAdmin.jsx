...
/**
 * @file The editing half of `<EditableList>`, reached through a dynamic import
 * so a visitor's bundle carries none of it.
 *
 * Everything here needs a session: the per-item ring and action row, the
 * pointer-driven reorder engine, the position field, the ghost add slot, the
 * label chip and the panel's wording. The public half has already resolved the
 * items and hands them over.
 */
...
const DIRECTION_LABEL = {
  up: "core.item.moveUp", down: "core.item.moveDown",
  left: "core.item.moveLeft", right: "core.item.moveRight",
};
 ...
 * }} props
 */
export function EditableListAdmin({
  fullPath, itemSchema, items, block, hasLocalDraft, localDraft,
  showInlineAdd, as, rest, children,
}) {
  const { uiStore, setDraft, setActiveBlock, setActiveListItem } = useCmsContext();

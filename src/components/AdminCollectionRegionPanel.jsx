...
function CreateForm({ collectionKey, schema, listParams }) {
  ...
  // Share the unfiltered list window so the sentinel lookup hits the cache
  // instead of a separate GET.
  const { items: unfiltered } = useCollection(collectionKey, listParams);
  const draftEntry = useMemo(
    // Only the slug-less sentinel is the new-item draft; a slug-bearing boot-GUID
    // row is a RoleDerived virtual item owned by RegionItemCard.
    () => unfiltered.find((row) => row.id === NEW_DRAFT_GUID && !row.slug) ?? null,
    [unfiltered],
  );

...
describe("declaring a block from the hook", () => {
  it("registers the vocabulary it was given", () => {
    ...
    const { getByTestId } = render(
      <CmsProvider config={{ baseUrl: BASE }} isAdmin initialSite={{ pages: [{ slug: "/", blocks: [block("meta.durum")] }], global: [] }}>
  ...
  it("registers nothing outside admin, where no drawer reads it", () => {
    ...
    const { getByTestId } = render(
      <CmsProvider config={{ baseUrl: BASE }} initialSite={{ pages: [{ slug: "/", blocks: [block("meta.durum")] }], global: [] }}>
        <Probe blockPath="meta.durum" />
        <Declarer />
      </CmsProvider>,
    );

 ...
 * request differs (see `createRecord`); the draft slot is the same slug-less
 * one either way:
 *
 ...
 *     is what the version guard catches.
 *
 * Writes one language. `useMultilingualCreate` writes several at once.
 */
...
export function useCollectionCreate({
  ...
  const submit = (onCreated) => {
    ...
    startTransition(async () => {
      try {
        const token = await getAccessToken();
        const created = await createRecord(config.transport, collectionKey, {
          data: buildPayload(schema.fields, values),
          slug: needsSlug ? ownSlug : undefined,
          locale,
        ...
        setError(createErrorMessage(err, { fields: schema.fields, needsSlug, t }));
      }
    });
  };

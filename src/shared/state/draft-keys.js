 ...
 * Translation edits are staged against a route the editor is not on, so that
 * route's store key (`routeKey(slug, locale)`) has to be part of this one.
 * Without it, navigating from `/a` to `/b` would hand a half-typed translation
 * of `/a`'s `hero.title` to `/b`'s.
 *
 * U+001F (unit separator) separates them because both halves are user-shaped:
 * a slug can hold a colon and a blockPath holds dots and brackets, so every
 * printable delimiter is claimable by one side or the other. Not NUL: that
 * would make git treat this file as binary and stop diffing it. The route key
 * joins its own halves with U+001E, so the first U+001F is always this one.
 *
 * @param {string} routeKey  The *target* route's store key (the other language's).
 * @param {string} blockPath
 * @returns {string}
 */
export function translationDraftKey(routeKey, blockPath) {
  return `${routeKey}\u001F${blockPath}`;
}

/**
 * @param {string} key
 * @returns {{ routeKey: string, blockPath: string } | null}
 */
export function parseTranslationDraftKey(key) {
  const i = key.indexOf("\u001F");
  if (i === -1) return null;
  return { routeKey: key.slice(0, i), blockPath: key.slice(i + 1) };
}

...
export function useCollectionEditor(collection, slug, { active = true, mirror = true, scopeId } = {}) {
  ...
  useEffect(() => {
    ...
    const queueKey = isVirtualNow
      ? newDraftKey(collection, locale)
      : itemDraftKey(collection, slug);
      ...
      try {
        ...
        if (isVirtualNow) {
          ...
          const body = slugSource === "AutoGenerated"
            ? { data: payload }
            : { slug, data: payload };
          await config.transport.saveCollectionNewDraft(collection, body, {
            accessToken: token,
      ...
      try {
        ...
        draftQueue.enqueue(queueKey, async () => {
          try {
            const token = await getAccessToken();
            if (isVirtualNow) {
              await config.transport.deleteCollectionNewDraft(collection, {
                accessToken: token,
                locale: locale ?? undefined,
              });
            } else {
